...
bool DbService::createProductionLine(const ProductionLine& line) {
    if (!ensureConnected()) return false;

    QSqlDatabase db = getDatabase();
    QSqlQuery query(db);

    query.prepare("INSERT INTO production_lines (name, description) VALUES (:name, :desc)");
    query.bindValue(":name", line.name);
    query.bindValue(":desc", QVariant());
...
bool DbService::updateProductionLine(const ProductionLine& line) {
    if (!ensureConnected()) return false;

    QSqlDatabase db = getDatabase();
    QSqlQuery query(db);

    query.prepare("UPDATE production_lines SET name = :name WHERE id = :id");
    query.bindValue(":name", line.name);
    query.bindValue(":id", line.id);
...
bool DbService::deleteProductionLine(ProductionLineId id) {
    if (!ensureConnected()) return false;

    QSqlDatabase db = getDatabase();
    QSqlQuery query(db);

    query.prepare("DELETE FROM production_lines WHERE id = :id");
    query.bindValue(":id", id);
...
QFuture<ImportResult> DbService::importItemsAsync(const QString& filePath,
                                                   ...
                                                   ProductId productId) {
    return QtConcurrent::run([this, filePath, lineId, productId]() {
        return doImportItems(filePath, lineId, productId);
    });
}
...
QFuture<ImportResult> DbService::importBoxesAsync(const QString& filePath,
    ...
    return QtConcurrent::run([this, filePath, lineId, packagingId]() {
        return doImportBoxes(filePath, lineId, packagingId);
    });
}
...
QFuture<ImportResult> DbService::importPalletsAsync(const QString& filePath,
    ...
    return QtConcurrent::run([this, filePath, lineId]() {
        return doImportPallets(filePath, lineId);
    });
}
...
QString DbService::getItemsTableName(const QString& productGtin) const {
    return QString("items_%1").arg(productGtin);
}

QString DbService::getBoxesTableName(const QString& packagingGtin) const {
    return QString("boxes_%1").arg(packagingGtin);
}

QString DbService::getAssignmentsTableName(const QString& productGtin, const QString& packagingGtin) const {
    return QString("%1_%2_assignments").arg(productGtin, packagingGtin);
}

QString DbService::getProductGtin(ProductId productId) {
    if (!ensureConnected()) return QString();

    QSqlDatabase db = getDatabase();
    QSqlQuery query(db);
    query.prepare("SELECT gtin FROM products WHERE id = :id");
...
QString DbService::getPackagingGtin(ProductPackagingId packagingId) {
    if (!ensureConnected()) return QString();

    QSqlDatabase db = getDatabase();
    QSqlQuery query(db);
    query.prepare("SELECT gtin FROM product_packaging WHERE id = :id");
    query.bindValue(":id", packagingId);
...
std::optional<ProductId> DbService::getProductIdForPackaging(ProductPackagingId packagingId) {
    if (!ensureConnected()) return std::nullopt;

    QSqlDatabase db = getDatabase();
    QSqlQuery query(db);
    query.prepare("SELECT product_id FROM product_packaging WHERE id = :id");
    query.bindValue(":id", packagingId);
...
bool DbService::createItemsTable(const QString& gtin, QSqlDatabase& db) {
    QString tableName = getItemsTableName(gtin);
    ...
    QString sql = QString(
        ...
        "    CONSTRAINT uniq_%1_barcode UNIQUE (bar_code),"
        "    CONSTRAINT fk_%1_production_line "
        "        FOREIGN KEY (production_line) REFERENCES production_lines(id)"
        "        ON DELETE RESTRICT ON UPDATE CASCADE,"
        "    CONSTRAINT chk_%1_status CHECK (status IN (0, 1, 2)),"
        "    CONSTRAINT chk_%1_barcode_not_empty CHECK (LENGTH(TRIM(bar_code)) > 0)"
        ")"
    ).arg(tableName);

    QSqlQuery query(db);
    if (!query.exec(sql)) {
        qWarning() << "Failed to create items table" << tableName << ":" << query.lastError().text();
        return false;
    }

    // Create indexes
    query.exec(QString("CREATE INDEX IF NOT EXISTS idx_%1_barcode ON %1(bar_code)").arg(tableName));
    query.exec(QString("CREATE INDEX IF NOT EXISTS idx_%1_status ON %1(status)").arg(tableName));
    query.exec(QString("CREATE INDEX IF NOT EXISTS idx_%1_production_status ON %1(production_line, status)").arg(tableName));

    return true;
}

bool DbService::createBoxesTable(const QString& gtin, QSqlDatabase& db) {
    QString tableName = getBoxesTableName(gtin);
    ...
    QSqlQuery query(db);
    if (!query.exec(sql)) {
        qWarning() << "Failed to create boxes table" << tableName << ":" << query.lastError().text();
        return false;
    }
...
bool DbService::createAssignmentsTable(const QString& productGtin, const QString& packagingGtin, QSqlDatabase& db) {
    QString tableName = getAssignmentsTableName(productGtin, packagingGtin);
    QString itemsTable = getItemsTableName(productGtin);
    QString boxesTable = getBoxesTableName(packagingGtin);
    ...
    QString sql = QString(
        ...
        "    CONSTRAINT uniq_%1_item UNIQUE (item_id),"
        "    CONSTRAINT fk_%1_item "
        "        FOREIGN KEY (item_id) REFERENCES %2(id)"
        "        ON DELETE CASCADE ON UPDATE CASCADE,"
        "    CONSTRAINT fk_%1_box "
        "        FOREIGN KEY (box_id) REFERENCES %3(id)"
        "        ON DELETE RESTRICT ON UPDATE CASCADE"
        ")"
    ).arg(tableName, itemsTable, boxesTable);

    QSqlQuery query(db);
    if (!query.exec(sql)) {
        qWarning() << "Failed to create assignments table" << tableName << ":" << query.lastError().text();
        return false;
    }

    // Create indexes
    query.exec(QString("CREATE INDEX IF NOT EXISTS idx_%1_item ON %1(item_id)").arg(tableName));
    query.exec(QString("CREATE INDEX IF NOT EXISTS idx_%1_box ON %1(box_id)").arg(tableName));
...
ImportResult DbService::doImportItems(const QString& filePath, ProductionLineId lineId, ProductId productId) {
    ImportResult result;

    // Get product GTIN
    QString productGtin = getProductGtin(productId);
    if (productGtin.isEmpty()) {
        result.errors.append("Product not found or invalid");
        return result;
    }

    QString tableName = getItemsTableName(productGtin);
    ...
    if (!db.isOpen()) {
        result.errors.append("Failed to create database connection");
        return result;
    }
    ...
    for (int i = 0; i < barcodes.size(); i += batchSize) {
        ...
        QString sql = QString(
            "INSERT INTO %1 (bar_code, production_line, status, imported_at) "
            "VALUES %2 ON CONFLICT (bar_code) DO NOTHING"
        ).arg(tableName, valuePlaceholders.join(", "));
    ...
    qDebug() << "DbService: Items import complete -" << result.summary();
    return result;
}

ImportResult DbService::doImportBoxes(const QString& filePath, ProductionLineId lineId, ProductPackagingId packagingId) {
    ImportResult result;

    // Get packaging GTIN
    QString packagingGtin = getPackagingGtin(packagingId);
    if (packagingGtin.isEmpty()) {
        result.errors.append("Packaging not found or invalid");
        return result;
    }

    QString tableName = getBoxesTableName(packagingGtin);
    ...
    qDebug() << "DbService: Boxes import complete -" << result.summary();
    return result;
}

ImportResult DbService::doImportPallets(const QString& filePath, ProductionLineId lineId) {
    ImportResult result;
    ...
    for (int i = 0; i < barcodes.size(); i += batchSize) {
        ...
        QString sql = QString(
            "INSERT INTO pallets (bar_code, production_line, status, created_at) "
            "VALUES %1 ON CONFLICT (bar_code) DO NOTHING"
    ...
    qDebug() << "DbService: Pallets import complete -" << result.summary();
    return result;
}
...
std::optional<Item> DbService::getItem(ProductId productId, ItemId id) {
    if (!ensureConnected()) return std::nullopt;
...
QVector<Item> DbService::getItemsByStatus(ProductId productId, ItemStatus status, 
                                           ProductionLineId lineId, int limit) {
    QVector<Item> items;

    if (!ensureConnected()) return items;

    QString productGtin = getProductGtin(productId);
    if (productGtin.isEmpty()) return items;
...
QVector<Item> DbService::getItemsInBox(ProductId productId, ProductPackagingId packagingId, BoxId boxId) {
    QVector<Item> items;
    ...
    QString productGtin = getProductGtin(productId);
    QString packagingGtin = getPackagingGtin(packagingId);
    if (productGtin.isEmpty() || packagingGtin.isEmpty()) return items;

    QString itemsTable = getItemsTableName(productGtin);
    QString boxesTable = getBoxesTableName(packagingGtin);
    QString assignmentsTable = getAssignmentsTableName(productGtin, packagingGtin);
    ...
    query.prepare(QString(
        ...
        "FROM %1 i "
        "JOIN %2 iba ON i.id = iba.item_id "
        "JOIN %3 b ON iba.box_id = b.id "
        "WHERE b.id = :boxId ORDER BY iba.assigned_at"
    ).arg(itemsTable, assignmentsTable, boxesTable));
...
QVector<Item> DbService::getScannedItemsNotInBox(ProductId productId, ProductPackagingId packagingId,
                                                  ProductionLineId lineId, int limit) {
    QVector<Item> items;
    ...
    QString sql = QString(
        "SELECT i.id, i.bar_code, i.status, i.production_line, i.imported_at, i.scanned_at "
        "FROM %1 i "
        "LEFT JOIN %2 iba ON i.id = iba.item_id "
        "WHERE i.status = 1 AND iba.item_id IS NULL"
    ).arg(itemsTable, assignmentsTable);
...
int DbService::countScannedItemsNotInBox(ProductId productId, ProductPackagingId packagingId,
                                          ProductionLineId lineId) {
    if (!ensureConnected()) return 0;

    QString productGtin = getProductGtin(productId);
    QString packagingGtin = getPackagingGtin(packagingId);
    if (productGtin.isEmpty() || packagingGtin.isEmpty()) return 0;
...
bool DbService::assignItemToBox(ProductId productId, ProductPackagingId packagingId,
    ...
    if (productGtin.isEmpty() || packagingGtin.isEmpty()) {
        QMutexLocker locker(&mutex_);
        lastError_ = "Invalid product or packaging";
        return false;
    }
    ...
    boxQuery.prepare(QString("SELECT id, status FROM %1 WHERE id = :id").arg(boxesTable));
    ...
    itemQuery.prepare(QString("SELECT id, status FROM %1 WHERE id = :id").arg(itemsTable));
    ...
    assignQuery.prepare(QString(
        "INSERT INTO %1 (item_id, box_id, assigned_at) "
        "VALUES (:itemId, :boxId, NOW())"
    ).arg(assignmentsTable));
    ...
    // Update item status
    QSqlQuery updateQuery(db);
    updateQuery.prepare(QString("UPDATE %1 SET status = 1 WHERE id = :id").arg(itemsTable));
...
int DbService::assignItemsToBox(ProductId productId, ProductPackagingId packagingId,
                                 const QVector<ItemId>& itemIds, BoxId boxId) {
    int count = 0;
    for (ItemId itemId : itemIds) {
        if (assignItemToBox(productId, packagingId, itemId, boxId)) {
            count++;
        }
    }
    return count;
...
std::optional<Box> DbService::getBox(ProductPackagingId packagingId, BoxId id) {
    if (!ensureConnected()) return std::nullopt;

    QString packagingGtin = getPackagingGtin(packagingId);
    if (packagingGtin.isEmpty()) return std::nullopt;
...
QVector<Box> DbService::getBoxesByStatus(ProductPackagingId packagingId, BoxStatus status,
                                          ProductionLineId lineId, int limit) {
    QVector<Box> boxes;
    ...
    QString packagingGtin = getPackagingGtin(packagingId);
    if (packagingGtin.isEmpty()) return boxes;
...
QVector<Box> DbService::getSealedBoxesNotOnPallet(ProductPackagingId packagingId,
                                                   ProductionLineId lineId, int limit) {
    QVector<Box> boxes;
...
int DbService::countSealedBoxesNotOnPallet(ProductPackagingId packagingId, ProductionLineId lineId) {
    if (!ensureConnected()) return 0;

    QString packagingGtin = getPackagingGtin(packagingId);
    if (packagingGtin.isEmpty()) return 0;
...
QVector<Box> DbService::getBoxesOnPallet(ProductPackagingId packagingId, PalletId palletId) {
    QVector<Box> boxes;
...
bool DbService::sealBox(ProductPackagingId packagingId, BoxId id) {
    if (!ensureConnected()) return false;

    QString packagingGtin = getPackagingGtin(packagingId);
    if (packagingGtin.isEmpty()) {
        QMutexLocker locker(&mutex_);
        lastError_ = "Invalid packaging";
        return false;
    }

    // Get product ID for this packaging to access assignments table
    auto productIdOpt = getProductIdForPackaging(packagingId);
    if (!productIdOpt) {
        QMutexLocker locker(&mutex_);
        lastError_ = "Could not determine product for packaging";
        return false;
    }

    QString productGtin = getProductGtin(*productIdOpt);
    QString boxesTable = getBoxesTableName(packagingGtin);
    QString assignmentsTable = getAssignmentsTableName(productGtin, packagingGtin);
    ...
    countQuery.prepare(QString(
        "SELECT COUNT(*) FROM %1 iba "
        "JOIN %2 b ON iba.box_id = b.id WHERE b.id = :id"
    ).arg(assignmentsTable, boxesTable));
    ...
    QSqlQuery query(db);
    query.prepare(QString(
        "UPDATE %1 SET status = 1, sealed_at = NOW() "
...
bool DbService::assignBoxToPallet(ProductPackagingId packagingId, BoxId boxId, PalletId palletId) {
    if (!ensureConnected()) return false;
    ...
    // Create assignment (using existing pallet_box_assignments table)
    QSqlQuery assignQuery(db);
    assignQuery.prepare(
...
int DbService::getBoxItemCount(ProductId productId, ProductPackagingId packagingId, BoxId id) {
    if (!ensureConnected()) return 0;
...
QFuture<ExportResult> DbService::exportItemsAsync(ProductId productId,
                                                   const QVector<ItemId>& itemIds,
                                                   const QString& lpTin) {
    return QtConcurrent::run([this, productId, itemIds, lpTin]() {
        return doExportItems(productId, itemIds, lpTin);
    });
}

QFuture<ExportResult> DbService::exportBoxesAsync(ProductId productId,
                                                   ProductPackagingId packagingId,
                                                   const QVector<BoxId>& boxIds,
                                                   const QString& lpTin) {
    return QtConcurrent::run([this, productId, packagingId, boxIds, lpTin]() {
        return doExportBoxes(productId, packagingId, boxIds, lpTin);
    });
}

QFuture<ExportResult> DbService::exportPalletsAsync(ProductId productId,
                                                     ProductPackagingId packagingId,
                                                     const QVector<PalletId>& palletIds,
                                                     const QString& lpTin) {
    return QtConcurrent::run([this, productId, packagingId, palletIds, lpTin]() {
        return doExportPallets(productId, packagingId, palletIds, lpTin);
    });
}

ExportResult DbService::doExportItems(ProductId productId, const QVector<ItemId>& itemIds, const QString& lpTin) {
    ExportResult result;
    ...
    // Verify items have status = 1 (Assigned/Scanned)
    QSqlQuery verifyQuery(db);
    QString verifySql = QString(
        "SELECT COUNT(*) FROM %1 WHERE id IN (%2) AND status = 1"
    ).arg(itemsTable, idList);
    ...
    if (verifyQuery.value(0).toInt() != itemIds.size()) {
        db.rollback();
        result.error = "Some items not found or not scanned";
        return result;
    }
    ...
    QString snapshotSql = QString(
        ...
        "SELECT %1, bar_code, NOW() FROM %2 WHERE id IN (%3)"
    ).arg(result.documentId).arg(itemsTable, idList);
    ...
    QString updateSql = QString(
        "UPDATE %1 SET status = 2 WHERE id IN (%2)"
    ).arg(itemsTable, idList);
...
ExportResult DbService::doExportBoxes(ProductId productId, ProductPackagingId packagingId,
    ...
    QString packagingGtin = getPackagingGtin(packagingId);
    if (productGtin.isEmpty() || packagingGtin.isEmpty()) {
        result.error = "Product or packaging not found";
        return result;
    }
    ...
    QString snapshotSql = QString(
        ...
        "SELECT %1, bar_code, NOW() FROM %2 WHERE id IN (%3)"
    ).arg(result.documentId).arg(boxesTable, idList);
    ...
    QString itemSnapshotSql = QString(
        ...
        "FROM %2 i "
        "JOIN %3 iba ON i.id = iba.item_id "
        "WHERE iba.box_id IN (%4)"
    ).arg(result.documentId).arg(itemsTable, assignmentsTable, idList);
    ...
    QString updateItemsSql = QString(
        "UPDATE %1 SET status = 2 WHERE id IN ("
        "  SELECT item_id FROM %2 WHERE box_id IN (%3))"
    ).arg(itemsTable, assignmentsTable, idList);
    ...
    qDebug() << "DbService: Box export complete - Doc:" << result.documentId;
...
ExportResult DbService::doExportPallets(ProductId productId, ProductPackagingId packagingId,
    ...
    QString itemSnapshotSql = QString(
        ...
        "FROM %2 i "
        "JOIN %3 iba ON i.id = iba.item_id "
        "JOIN %4 b ON iba.box_id = b.id "
        "JOIN pallet_box_assignments pba ON b.id = pba.box_id "
        "WHERE pba.pallet_id IN (%5)"
    ).arg(result.documentId).arg(itemsTable, assignmentsTable, boxesTable, idList);
    ...
    QString updateItemsSql = QString(
        "UPDATE %1 SET status = 2 WHERE id IN ("
        "  SELECT i.id FROM %1 i "
        "  JOIN %2 iba ON i.id = iba.item_id "
        "  JOIN %3 b ON iba.box_id = b.id "
        "  JOIN pallet_box_assignments pba ON b.id = pba.box_id "
        "  WHERE pba.pallet_id IN (%4))"
    ).arg(itemsTable, assignmentsTable, boxesTable, idList);
...
ProductionStats DbService::getStats(ProductId productId, ProductPackagingId packagingId,
                                     std::optional<ProductionLineId> lineId) {
    ...
    QString productGtin = getProductGtin(productId);
    QString packagingGtin = getPackagingGtin(packagingId);
    if (productGtin.isEmpty() || packagingGtin.isEmpty()) return stats;
    ...
    if (itemsQuery.exec(QString("SELECT status, COUNT(*) FROM %1%2 GROUP BY status").arg(itemsTable, where))) {
    ...
    if (boxesQuery.exec(QString("SELECT status, COUNT(*) FROM %1%2 GROUP BY status").arg(boxesTable, where))) {
...
bool DbService::createProduct(const Product& product) {
    ...
    // Create the items table for this product
    if (!createItemsTable(product.gtin, db)) {
        db.rollback();
        QMutexLocker locker(&mutex_);
        lastError_ = "Failed to create items table for product";
        return false;
    }
...
bool DbService::createPackaging(const ProductPackaging& pkg) {
    ...
    // Get the product GTIN for creating the assignments table
    QString productGtin = getProductGtin(pkg.productId);
    if (productGtin.isEmpty()) {
        QMutexLocker locker(&mutex_);
        lastError_ = "Product not found";
        return false;
    }
    ...
    // Create the boxes table for this packaging
    if (!createBoxesTable(pkg.gtin, db)) {
        db.rollback();
        QMutexLocker locker(&mutex_);
        lastError_ = "Failed to create boxes table for packaging";
        return false;
    }

    // Create the item-box assignments table for this product/packaging combination
    if (!createAssignmentsTable(productGtin, pkg.gtin, db)) {
        db.rollback();
        QMutexLocker locker(&mutex_);
        lastError_ = "Failed to create assignments table";
        return false;
    }
...
// These use the legacy single items/boxes/item_box_assignments tables
// ============================================================================

QFuture<ImportResult> DbService::importItemsAsync(const QString& filePath, ProductionLineId lineId) {
    return QtConcurrent::run([this, filePath, lineId]() {
        return doImportLegacy(filePath, lineId, "items");
    });
}

QFuture<ImportResult> DbService::importBoxesAsync(const QString& filePath, ProductionLineId lineId) {
    return QtConcurrent::run([this, filePath, lineId]() {
        return doImportLegacy(filePath, lineId, "boxes");
    });
}

ImportResult DbService::doImportLegacy(const QString& filePath, ProductionLineId lineId,
    ...
    qDebug() << "DbService: Legacy import complete -" << result.summary();
    return result;
}
...
QVector<Item> DbService::getItemsByStatus(ItemStatus status, ProductionLineId lineId, int limit) {
    QVector<Item> items;
...
QVector<Box> DbService::getBoxesByStatus(BoxStatus status, ProductionLineId lineId, int limit) {
    QVector<Box> boxes;
...
QFuture<ExportResult> DbService::exportItemsAsync(const QVector<ItemId>& itemIds, const QString& lpTin) {
    return QtConcurrent::run([this, itemIds, lpTin]() {
        return doExportItemsLegacy(itemIds, lpTin);
    });
}

QFuture<ExportResult> DbService::exportBoxesAsync(const QVector<BoxId>& boxIds, const QString& lpTin) {
    return QtConcurrent::run([this, boxIds, lpTin]() {
        return doExportBoxesLegacy(boxIds, lpTin);
    });
}

QFuture<ExportResult> DbService::exportPalletsAsync(const QVector<PalletId>& palletIds, const QString& lpTin) {
    return QtConcurrent::run([this, palletIds, lpTin]() {
        return doExportPalletsLegacy(palletIds, lpTin);
    });
}

ExportResult DbService::doExportItemsLegacy(const QVector<ItemId>& itemIds, const QString& lpTin) {
    ExportResult result;
    ...
    qDebug() << "DbService: Legacy item export complete - Doc:" << result.documentId;
...
ExportResult DbService::doExportBoxesLegacy(const QVector<BoxId>& boxIds, const QString& lpTin) {
    ExportResult result;
    ...
    qDebug() << "DbService: Legacy box export complete - Doc:" << result.documentId;
...
ExportResult DbService::doExportPalletsLegacy(const QVector<PalletId>& palletIds, const QString& lpTin) {
    ExportResult result;
    ...
    qDebug() << "DbService: Legacy pallet export complete - Doc:" << result.documentId;
